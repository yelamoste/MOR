...
$uid = $_SESSION['view-research-title-id3'];

echo $uid;

// echo $uid. "wth";

$sql10 = "SELECT tp3.id, tp2.id, tp3.thesis3, tp2.thesis, tb.id, tb.title_proposal_id, tp.id, tp.title, tp.filename, tp3.response, tb.research_adviser, tp3.adviser_response, f.id, f.faculty_name
...
$sql11 = "SELECT p.id , p.panelist, p.thesis, p.response, f.id, tp2.id, tp2.thesis,tb.id, tb.title_proposal_id, tp.id, f.faculty_name
FROM panelists p
INNER JOIN faculty_users AS f ON p.panelist = f.id
INNER JOIN title_proposals_2 AS tp2 ON p.thesis = tp2.id
INNER JOIN thesis_basic_info AS tb ON tp2.thesis = tb.id
INNER JOIN title_proposals AS tp ON tb.title_proposal_id = tp.id WHERE user_id = $id AND title_proposal_id = $uid";
...
if ($result10->num_rows > 0) {
    while ($row = $result10->fetch_assoc()) {
        ...
        $ovaresponse = $row['adviser_response'];
            ...
            <div class="comment-recommendation-cont">
                ...
                <form action="" method="POST">
                    ...
                    if ($ovaresponse == "Approved" or $ovaresponse == "Conditional") {
                        ...
                        echo "<div class='buttons-view-proceed'><p class='response-ind' style='color:#DC8116;'> Please wait for the Research Adviser's and Research Panelists' responses..</div>";
                    } else {
